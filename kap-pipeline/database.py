...
class CashFlow(Base):
    ...
    __table_args__ = (
        UniqueConstraint("company_id", "year", "period", name="uq_cashflow_company_period"),
        Index("ix_cf_company_year", "company_id", "year"),
    )
...
# ─── Module 9: Management Board ─────────────────────────────────────────────
class ManagementMember(Base):
    __tablename__ = "management_members"
    __table_args__ = (
        Index("ix_mgmt_company", "company_id"),
    )
    ...
    member_type: Mapped[Optional[str]] = Column(String(50))  # chairman, ceo, cfo, member, independent
    created_at: Mapped[datetime] = Column(DateTime, default=datetime.utcnow)
...
class Subsidiary(Base):
    ...
    __table_args__ = (
        Index("ix_sub_company", "company_id"),
    )
    ...
    activity: Mapped[Optional[str]] = Column(String(500))
    relation_type: Mapped[Optional[str]] = Column(String(50))  # subsidiary, affiliate, investment
    created_at: Mapped[datetime] = Column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)


# ─── Module 11: Portfolio Reports ───────────────────────────────────────────
class PortfolioReport(Base):
    __tablename__ = "portfolio_reports"
    __table_args__ = (
        UniqueConstraint("disclosure_id", "security_name", name="uq_pdr_disclosure_security"),
    )
...
class FinancialNote(Base):
    ...
    __table_args__ = (
        UniqueConstraint("disclosure_id", name="uq_financial_note"),
    )
    ...
    note_type: Mapped[Optional[str]] = Column(String(50))  # financial_note, risk_report, etc.
